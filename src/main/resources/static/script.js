...
const ENDPOINTS = {
  ...
  USER_STATUS: `${API_BASE_URL}/userStatuses`,
  BINARY_CONTENT: `${API_BASE_URL}/binaryContents/check`
};
...
async function fetchAndRenderUsers() {
  try {
    ...
    if (!userStatusesResponse.ok) {
      throw new Error(
          `Failed to fetch userStatuses: ${userStatusesResponse.status}`);
    }
...
async function renderUserList(users, userStatuses, profiles) {
  ...
  for (const user of users) {
    ...
    console.log("User Status Map has userStatusId:",
        userStatusMap.has(String(user.userStatusId)));

    // 사용자 상태 가져오기
    const userStatus = userStatusMap.get(String(user.userStatusId));
    console.log(`User ${user.name} status:`, userStatus);
    console.log(`User ${user.name} online value:`,
        userStatus ? userStatus.online : "N/A");

    const isOnline = userStatus ? userStatus.online : false; // 기본값: 오프라인

    // 프로필 이미지 URL 가져오기
    console.log(`User ${user.name} profileId:`, user.profileImageId);
    console.log("Profile Map has profileId:",
        profileMap.has(user.profileImageId));

    const profileUrl = profileMap.get(user.profileImageId)
        || '/default-avatar.png';
